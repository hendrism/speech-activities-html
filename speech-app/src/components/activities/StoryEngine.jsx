                ...
                <AnimatePresence mode="wait">
                    ...
                    >
                        {story.questions.map((q, idx) => (
                            <div key={`${currentStoryIndex}-${idx}`} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                                <h4 className="text-lg font-bold text-slate-800 mb-4 flex gap-3">
                                    <span className="bg-slate-100 text-slate-500 w-8 h-8 rounded-full flex items-center justify-center text-sm flex-shrink-0">
                                        {idx + 1}
                                    </span>
                                    {q.q}
                                </h4>
                                ...
                                <ProgressiveHelp
                                    ...
                                    hint="Look back at the story text for the answer." // Generic hint for now
                                    onComplete={() => { }}
                                />
                            </div>
                        ))}
